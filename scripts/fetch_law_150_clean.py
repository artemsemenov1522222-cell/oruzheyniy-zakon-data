...
class ArticleBodyParser(HTMLParser):
    """
    Extracts visible text beginning at the matching <h1> article heading.

    This avoids breadcrumbs, site navigation, login links and other page
    chrome that appear before the actual legal text.
    """
    ...
    def handle_starttag(
        ...
        if self._capture and lowered in self.BREAK_TAGS:
            self._parts.append("\n")
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if lowered == "h1":
            heading = normalize_inline(
                " ".join(self._h1_parts)
            )

            expected = re.compile(
                rf"^Статья\s+{re.escape(self.article_number)}\.",
                flags=re.IGNORECASE,
            )

            if expected.match(heading):
                self._capture = True
...
def find_article_start(
    ...
    if exact_positions:
        # Breadcrumbs usually contain the first occurrence; the actual H1
        # is the later occurrence before the legal body.
        return exact_positions[-1]
...
def remove_consultant_service_blocks(
    ...
    service_patterns = (
        ...
        re.compile(
            r"^Подготовлена редакция документа.*$",
            flags=re.IGNORECASE,
        ),
        re.compile(
            r"^Федеральный закон от 13\.12\.1996.*150-ФЗ.*$",
            flags=re.IGNORECASE,
        ),
        re.compile(
            r'^["«]?Об оружии["»]?$',
    ...
    skip_next_note_paragraph = False
    ...
    for line in lines:
        ...
        if stripped in exact_service_lines:
            continue
        ...
        if re.match(
            ...
            skip_next_note_paragraph = True
            continue

        if skip_next_note_paragraph:
            # Consultant notes are editorial, not the statutory text.
            skip_next_note_paragraph = False
            continue
...
def clean_normative_text(
    source_text: str,
    article_number: str,
) -> str:
    """
    Keeps the current consolidated statutory wording and removes page
    navigation plus editorial amendment-history annotations.
    """
    result: list[str] = []

    heading_pattern = re.compile(
        rf"^Статья\s+{re.escape(article_number)}\.\s+",
        flags=re.IGNORECASE,
    )
    ...
    editorial_patterns = (
        ...
        re.compile(
            r"^\((?:часть|пункт|п\.|абзац|статья|редакция).+"
            r"(?:введен|введена|введены|в ред\.|утратил).*\)$",
            flags=re.IGNORECASE,
        ),
        re.compile(
            r"^\(Федеральный закон.*\)$",
...
def extract_article(
    ...
    candidate = body_parser.text()

    if not candidate:
        # Defensive fallback for a future page-layout change.
    ...
    source_text = remove_consultant_service_blocks(
        source_text
    )

    clean_text = clean_normative_text(
        source_text,
        article["number"],
    )
    ...
    leaked = [
        ...
        if fragment.lower() in clean_text.lower()
    ]

    if leaked:
        raise RuntimeError(
            "В очищенный текст попали служебные блоки: "
            + ", ".join(leaked)
        )
