...
def extract_rg_document(text: str) -> str:
    """
    Выделяет именно текст нормативного акта со страницы «Российской газеты».

    Структура страницы и кавычки в заголовке могут меняться, поэтому
    извлечение опирается не на один точный заголовок, а на устойчивые
    юридические маркеры: реквизиты, статьи и подпись Президента.
    """
    normalized = normalize_text(text)

    title_markers = (
        "Федеральный закон от 4 июля 2026 г. N 236-ФЗ",
        "Федеральный закон от 4 июля 2026 г. № 236-ФЗ",
        "Федеральный закон от 4 июля 2026 года N 236-ФЗ",
        "Федеральный закон от 4 июля 2026 года № 236-ФЗ",
    )
    ...
    article_one = re.search(
        r"(?:^|\n)Статья\s+1(?:\s|\n)",
        normalized,
        flags=re.IGNORECASE,
    )

    if not start_candidates and article_one is None:
        raise RuntimeError(
            "На странице не найдены реквизиты закона или начало статьи 1"
        )

    if start_candidates:
        start = min(start_candidates)
    else:
        # Добавляем небольшой контекст перед статьёй 1.
        start = max(0, article_one.start() - 800)

    candidate = normalized[start:]

    # Основной надёжный конец документа — подпись Президента.
    signature = re.search(
        r"Президент Российской Федерации\s+В\.\s*Путин",
        candidate,
        flags=re.IGNORECASE,
    )
    ...
    if signature:
        ...
        # Резервный конец: после статьи 2, до служебных блоков сайта.
        footer_patterns = (
            r"(?:^|\n)Российская газета\s*-\s*Федеральный выпуск",
            r"(?:^|\n)Комментарии Российской Газеты",
            r"(?:^|\n)Рассылка свежих документов",
            r"(?:^|\n)Новые документы",
            r"(?:^|\n)©\s*1998-",
        )
        ...
        if end_candidates:
            candidate = candidate[:min(end_candidates)]
    ...
    if missing:
        raise RuntimeError(
            "Выделенный текст не прошёл проверку; отсутствуют: "
            + ", ".join(missing)
        )

    if len(candidate) < 3000:
        raise RuntimeError(
            "Выделенный текст закона подозрительно короткий: "
            f"{len(candidate)} символов"
        )
